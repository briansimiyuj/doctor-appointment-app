...
const VideoControls: React.FC = ()=>{
    ...
    return(
        ...
        <div className="absolute bottom-0 w-full p-2 sm:p-4 bg-gray-800/80 flex justify-between items-center z-10">
            
            <div className={`text-lg sm:text-xl font-mono px-2 sm:px-3 py-1 rounded-md transition-colors 
            ...
            <div className="flex items-center space-x-1 sm:space-x-2 md:space-x-3">
                ...
                <button 
                    onClick={toggleMic}
                    className={`p-2 sm:p-3 rounded-full text-white dark:text-white transition ${isMicMuted ? 'bg-red-600 hover:bg-red-700' : 'bg-gray-700 hover:bg-gray-600'}`}
                    title={isMicMuted ? "Unmute" : "Mute"}
                >

                    {

                        isMicMuted ? <FaMicrophoneSlash className="w-4 h-4 sm:w-5 sm:h-5"/> : <FaMicrophone className="w-4 h-4 sm:w-5 sm:h-5"/> 
                ...
                <button 
                    onClick={toggleCam}
                    className={`p-2 sm:p-3 rounded-full text-white dark:text-white transition ${isCamOff ? 'bg-red-600 hover:bg-red-700' : 'bg-gray-700 hover:bg-gray-600'}`}
                    title={isCamOff ? "Turn Video On" : "Turn Video Off"}
                >
   
                    { isCamOff ? <FaVideoSlash className="w-4 h-4 sm:w-5 sm:h-5"/> : <FaVideo className="w-4 h-4 sm:w-5 sm:h-5"/> }

                </button>

                <button 
                    onClick={toggleFullScreen}
                    className="p-2 sm:p-3 rounded-full text-white dark:text-white bg-gray-700 hover:bg-gray-600 transition"
